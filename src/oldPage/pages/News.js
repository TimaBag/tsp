...
import {  Row,
          ...
          Card,
          List,
          Icon,Pagination,DatePicker,Modal,Button } from 'antd';
...
class NewsPage extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    let paramsNewsID = this.props.match.params.item_id;
  ...
  filterNewsList(newsList){
    return newsList.filter(
      (news) => _.toLower(news.title).includes(_.toLower(this.state.query || ''))
    )
  }
      ...
      <div>
        ...
        <Row gutter={24}>
          <Col xl={13} lg={13} md={13} sm={24} xs={24} className="mb1">
            {(!(showNewsDetail && selectedNewsItem !== null) || window.screen.width > 900) && <Card
              title={
                <Row gutter={24} className="flex item-cnt">
                  ...
                  <Col xl={8} lg={8} md={8} sm={24} xs={24}>
                    <h2 className="fm-RobotoRegular fs-12 black-text txt-underline txt-link" onClick={this.handleShowSortDate}>Сортировка по дате</h2>
                  </Col>
            ...
            >
              <List
                ...
                renderItem={item => (
                  <List.Item
                    className={(selectedNewsItem !== null && item.id === selectedNewsItem.id) ? "activeNews" : ""}
                  >
                    <List.Item.Meta
              ...
              />
              <Pagination defaultCurrent={1} total={this.props.newsCount} onChange={this.handleChangePage} />
            </Card>
          }
          </Col>
          ...
          {(showNewsDetail && selectedNewsItem !== null) &&
                ...
                <Row gutter={24}>
                  <Col xl={24} lg={24} md={24} sm={24} xs={24}>
                    <div className="flex justify_between item-cnt">
                      <a onClick={() => this.closeNewsReader()} className="back-link"><Icon type="left" /> Назад</a>
                      <a onClick={() => this.closeNewsReader()} className="btn btn-close blue-text fs-21"><Icon type="close" /></a>
                    </div>
                  </Col>
                </Row>
              }
            ...
            >
              <div className="news-content">
                <span className="news-current_date blue-text">{extractTime(selectedNewsItem.datetime)}</span>
                <h2 className="news-current_title">{selectedNewsItem.title}</h2>
                <div className="news-content_desc">
                  {this.getNewsDesc(selectedNewsItem.text)}
                </div>
                <span className="source-loc gray-text">Источник новости: {selectedNewsItem.source}</span>
              </div>
            </Card>)
          }
          </Col>
        ...
        >
          ...
          <Row gutter={24} className="mt2">
            <Col xl={8} lg={8} md={8} sm={24} xs={24}>
              <Button className="btn btn-block btn-blue fm-RobotoMedium fs-15" onClick={this.handleSortDate}>Сортировать</Button>
            </Col>
            <Col xl={8} lg={8} md={8} sm={24} xs={24}>
              <Button className="btn btn-block btn-blue fm-RobotoMedium fs-15" onClick={this.handleCancelSortDate}>Отменить</Button>
            </Col>
          </Row>
        </Modal>
      </div>
